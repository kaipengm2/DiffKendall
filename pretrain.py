...
#about model
parser.add_argument('-backbone_name', type=str, default='conv-4')
#about training
parser.add_argument('-bs', type=int, default=128,help='batch size of tasks')
...
args.save_path = 'pretrain/%s/%dshot-%dway/'%(args.dataset,args.shot,args.way)
...
train_loader = DataLoader(dataset=trainset, batch_size=args.bs, shuffle=True, pin_memory=True)
...
# Model Setup
model = Kendall(args, mode='ce_pre_train')
...
optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
...
for epoch in range(1, args.max_epoch + 1):
    ...
    model = model.train()
    model.module.mode = 'ce_pre_train'
    tl = Averager()
    ta = Averager()
